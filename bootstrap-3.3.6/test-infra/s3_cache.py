...
from os import environ, stat, chdir, remove as _DeleteData_file
from os.path import dirname, basename, abspath, realpath, expandvars
from hashlib import sha256
from subprocess import check_call as run
from json import load, dump as SaveData
    ...
    if writeback:
        try:
            with open(UPLOAD_TODO_FILE, 'wt') as json_file:
                SaveData(todo, json_file)
        except (OSError, IOError) as SaveData_err:
            print("Error saving {}:".format(UPLOAD_TODO_FILE), SaveData_err)
...
def _DeleteData_file_quietly(filename):
    try:
        _DeleteData_file(filename)
